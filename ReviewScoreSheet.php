<!DOCTYPE html>
  <html lang = "en">
  <head>
    <meta charset="utf-8" />
    <meta name="description" content="ScoreSheet" />
    <meta name="keywords" 	 content="HTML, CSS, JS" />
    <meta name="author"		 content="Jinjuta" />
    <title> Review Score Sheet </title>
  </head>
    <style>
              .table
        {
          display:table;
          border-collapse:separate;
          border-spacing:2px;
          width: 500px;
        }
        .thead
        {
          display:table-header-group;
          color:white;
          font-weight:bold;
          background-color:grey;
        }
        .tbody
        {
          display:table-row-group;
        }
        .tr
        {
          display:table-row;
        }
        .td
        {
          display:table-cell;
          border:1px solid black;
          padding:1px;
        }
        .tr.editing .td input
        {
          width:100px;
        }
    </style>
  <body>
 	  <?php include 'includes/header.inc.php'; ?>
    <?php
    function fetchScore($conn, $endId, $rangeNum, $endNum, $arrowNum) {
      $query = "SELECT score_arrow{$arrowNum} as score 
                FROM EndScores
                WHERE end_id = '$endId' AND range_score_id = '$rangeNum' 
                AND end_number = '$endNum'";
      $result = mysqli_query($conn, $query);

      if ($result && mysqli_num_rows($result) > 0) {
          $row = mysqli_fetch_assoc($result);
          return $row['score'];
      }

      return 0; // return 0 if no score found
    }
    ?>

<!-- Queries print output from table here -->
    <?php
      error_reporting(E_ALL);
      ini_set('display_errors', 1);

      if (isset($_POST["Competition"])){
        $competition = ($_POST["Competition"]);
        $_SESSION["competition"] = $competition;
      }

      if (isset($_POST["Archer"])){
          $archerId = ($_POST["Archer"]);
          $_SESSION["archer"] = $archerId;
      }

      require_once "settings.php";
      $conn = @mysqli_connect($host,$user,$pwd,$sql_db);
      
          // find out how many ranges in competition's round
          $query = "SELECT COUNT(*) FROM RoundRange WHERE `round_def_id` IN
          (SELECT `round_def_id` FROM `Competitions` WHERE `competition_id` = '$competition') ";
          $RangeCount = mysqli_fetch_array(mysqli_query($conn, $query))[0];
          
          // find out competition's round
          $query = "SELECT `round_def_id` FROM `Competitions` WHERE `competition_id` = '$competition'";
          $Round = mysqli_fetch_array(mysqli_query($conn, $query))['round_def_id'];
          ?>
          <div class="container-sm">
          <h1 style="text-align: center">Review Score</h1>

          <?php
          $archerQuery = "SELECT `archer_firstname`, `archer_lastname` FROM `Archers` WHERE `archer_id` = '$archerId'";
          $archerResult = mysqli_query($conn, $archerQuery);

        if ($archerResult && mysqli_num_rows($archerResult) > 0) {
            $archer = mysqli_fetch_assoc($archerResult);
            $archerFirstName = $archer['archer_firstname'];
            $archerLastName = $archer['archer_lastname'];

            echo "<h2 style='text-align: center'> Archer: " . $archerFirstName . " " . $archerLastName . "</h2>";
        }
        else {
            echo "<h2> No Archer found with ID: $archerId </h2>";
        }
    ?>

<!-- The For loop starts here -->
   
    <?php
    for($RangeNum=1; $RangeNum<=$RangeCount; $RangeNum++)
    {
    $query = "SELECT `target_distance` FROM `RoundRange` WHERE `range_number` = '$RangeNum' AND `round_def_id` = '$Round'";
    $RangeDistance = mysqli_fetch_array(mysqli_query($conn, $query))['target_distance'];
      // Range details are printed here
      echo ' 
      <h2> Range ' . $RangeNum . ', ' . $RangeDistance . ' Metres</h2>
      <div class="table">
        <div class="thead">
          <div class="tr">
            <div class="td"></div>
            <div class="td">Arrow 1</div>
            <div class="td">Arrow 2</div>
            <div class="td">Arrow 3</div>
            <div class="td">Arrow 4</div>
            <div class="td">Arrow 5</div>
            <div class="td">Arrow 6</div>
          </div>
        </div>
      ';
        $query = "SELECT `end_count` FROM RoundRange WHERE `range_number` = $RangeNum AND `round_def_id` = '$Round'";
        $EndCount = mysqli_fetch_array(mysqli_query($conn, $query))['end_count'];
          echo ' 
            <div class="tbody">
          ';
          for($EndNum=1; $EndNum<=$EndCount; $EndNum++)
          {
            echo ' <form class="tr" method="POST" action="ReviewScoreSheet.php"> 
            <div class="thead">End ' . $EndNum . '</div>';
            
            for($arrowNum=1; $arrowNum<=6; $arrowNum++)
            {
              $arrowScore = fetchScore($conn, $archerId, $RangeNum, $EndNum, $arrowNum, $Round);
              echo '<div class="td"><input form="tr" type="text" name="' . $RangeNum .'_' . $EndNum . '_' . $arrowNum . '" value="' . $arrowScore . '" maxlength="2" size="2"/></div>';
            }
          
            echo '<input type="submit" value="APPROVE" name="submit" style="margin-top: 10px">
            </form>';
          }
          
         echo " 
                </div>
            </div>
          ";
    }
    ?>
    </div>
  </body>
</html>
